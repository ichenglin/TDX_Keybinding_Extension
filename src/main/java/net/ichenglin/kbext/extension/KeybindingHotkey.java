...
public class KeybindingHotkey {
    ...
    public void hotkey_register(int id, int modifiers, int keycode, boolean enabled, Runnable handler) {
        boolean hotkey_enabled = this.hotkey_enabled;
        if (hotkey_enabled) this.hotkey_toggle(false);
        this.hotkey_registry.put(id, new HotkeyRegistry(id, modifiers, keycode, enabled, handler));
        if (hotkey_enabled) this.hotkey_toggle(true);
    }

    public void hotkey_update(int id, Integer modifiers, Integer keycode, Boolean enabled, Runnable handler) {
        ...
        boolean  hotkey_enabled   = (enabled   == null) ? registry_data.hotkey_enabled   : enabled;
        Runnable hotkey_handler   = (handler   == null) ? registry_data.hotkey_handler   : handler;
        this.hotkey_register(id, hotkey_modifiers, hotkey_keycode, hotkey_enabled, hotkey_handler);
    }
...
class KeybindingHotkeyThread extends Thread {
    ...
    public void run() {
        ...
        for (Map.Entry<Integer, HotkeyRegistry> registry_entry : this.hotkey_registry.entrySet()) {
            ...
            if (!registry_data.hotkey_enabled) continue;
...
class HotkeyRegistry {
    ...
    public boolean  hotkey_enabled;
    public Runnable hotkey_handler;

    public HotkeyRegistry(int id, int modifiers, int keycode, boolean enabled, Runnable handler) {
        this.hotkey_id        = id;
